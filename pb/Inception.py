...
path = r'C:\workspace\胃镜部位识别（InceptionNet）\model\Inception.ckpt'
trainpath = [r'C:\workspace\胃镜部位识别（InceptionNet）\tfrecord\64_shuffle\train.tfrecords',
             r'C:\workspace\胃镜部位识别（InceptionNet）\tfrecord\64_shuffle\train_001.tfrecords']
testpath = [r'C:\workspace\胃镜部位识别（InceptionNet）\tfrecord\64_shuffle\test.tfrecords',
            r'C:\workspace\胃镜部位识别（InceptionNet）\tfrecord\64_shuffle\test_001.tfrecords']
validpath = [r'C:\workspace\胃镜部位识别（InceptionNet）\tfrecord\64_shuffle\validation.tfrecords']
...
#pb
x=tf.placeholder("float",[None,64,64,1],name='input')
with slim.arg_scope(inception_v3_arg_scope()):
    _, pb_infer = inceptionV3.inference(x, classnum)

out=tf.reshape(pb_infer['Predictions'],[-1,classnum],name='output')
...
def test(loop=1):
    ...
    with tf.Session(config=config) as sess:
        ...
        tf.train.Saver().restore(sess,path)
        for i in range(loop):
            test1 = sess.run([test_out],feed_dict=feed_dict)
        print('test1: ',test1[0])
        coord.request_stop()
        coord.join(threads)


def ckpt2pb(folder,filename):
    config=tf.ConfigProto()
    config.gpu_options.allow_growth=True
    ...
    with tf.Session(config=config) as sess:
        ...
        tf.train.write_graph(graph, folder,filename, as_text=False)
    print('done')

def prediction(pb_path, image_path):
...
ckpt2pb(r'C:\workspace\aaa','Inception.pb')
predicts = prediction(r'C:\workspace\aaa\Inception.pb', r'C:\Users\MEDCARE-OO\Desktop\train\154_three.jpg')
