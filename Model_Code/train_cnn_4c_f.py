...
def get_test_batch(image,label,batch_size):
    ...
    #images,labels=tf.train.shuffle_batch([image,label],
        #batch_size=batch_size,num_threads=10,capacity=1000,min_after_dequeue=300)
    return images,tf.reshape(labels,[batch_size])
...
class trainwork(object):
    def __init__(self):
        with tf.variable_scope('scop'):
            self.weights={
                'conv1':tf.get_variable('w1', [5,5,1,6],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'conv2':tf.get_variable('w2', [5,5,6,8],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'conv3':tf.get_variable('w3', [5,5,8,16],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'conv4':tf.get_variable('w4', [5,5,16,32],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'fc1':tf.get_variable('w5', [4*4*32,1024],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'out':tf.get_variable('w6', [1024,classnum],initializer=tf.contrib.layers.xavier_initializer_conv2d())
            }
            self.biases={
                'conv1': tf.get_variable('b1', [6],initializer=tf.constant_initializer(0.0)),
                'conv2': tf.get_variable('b2', [8],initializer=tf.constant_initializer(0.0)),
                'conv3': tf.get_variable('b3', [16],initializer=tf.constant_initializer(0.0)),
                'conv4': tf.get_variable('b4', [32],initializer=tf.constant_initializer(0.0)),
                'fc1': tf.get_variable('b5', [1024],initializer=tf.constant_initializer(0.0)),
                'out': tf.get_variable('b6', [classnum],initializer=tf.constant_initializer(0.0))
            }
    ...
    def inference(self,images):
        ...
        #print(pool2.shape)
        l3 = tf.nn.conv2d(pool2,self.weights['conv3'],strides=[1,1,1,1],padding='SAME')+self.biases['conv3']
        relu3=tf.nn.relu(l3)
        pool3=tf.nn.max_pool(relu3,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
        #print(pool3.shape)
        l4 = tf.nn.conv2d(pool3,self.weights['conv4'],strides=[1,1,1,1],padding='SAME')+self.biases['conv4']
        relu4=tf.nn.relu(l4)
        pool4=tf.nn.max_pool(relu4,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
        #print(pool4.shape)
        flat=tf.reshape(pool4,[-1,4*4*32])
        fc1=tf.matmul(flat, self.weights['fc1'])+self.biases['fc1']
        fc1_relu=tf.nn.relu(fc1)
        fc1_drop=tf.nn.dropout(fc1_relu,0.9)
