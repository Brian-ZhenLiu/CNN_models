...
trainpath=[r'C:\workspace\模型优化\train.tfrecords']
           #r'C:\workspace\Alexnet\tfrecord\train_001_L.tfrecords']
testpath=[r'C:\workspace\模型优化\test.tfrecords']
         # r'C:\workspace\Alexnet\tfrecord\test_001_L.tfrecords']
validpath = [r'C:\workspace\模型优化\validation.tfrecords']
...
class trainwork(object):
    def __init__(self):
        with tf.variable_scope('scop'):
            self.weights={
                'conv1':tf.get_variable('w1', [3,3,3,32],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'conv2':tf.get_variable('w2', [5,5,32,64],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'conv3':tf.get_variable('w3', [8,8,64,512],initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'out':tf.get_variable('w4', [512,classnum],initializer=tf.contrib.layers.xavier_initializer_conv2d())
            }
            self.biases={
                'conv1': tf.get_variable('b1', [32],initializer=tf.constant_initializer(0.0)),
                'conv2': tf.get_variable('b2', [64],initializer=tf.constant_initializer(0.0)),
                'conv3': tf.get_variable('b3', [512],initializer=tf.constant_initializer(0.0)),
    ...
    def inference(self,images):
        ...
        l3=tf.nn.conv2d(pool2,self.weights['conv3'],strides=[1,1,1,1],padding='VALID')+self.biases['conv3']
        relu3=tf.nn.relu(l3)
        relu3=tf.reshape(relu3,[-1,512])
        out=tf.matmul(relu3,self.weights['out'])+self.biases['out']
        return out
