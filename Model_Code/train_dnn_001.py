...
class trainwork(object):
    def __init__(self):
        with tf.variable_scope('scop'):
            self.w1=tf.get_variable('w1', [4096,classnum],initializer=tf.contrib.layers.xavier_initializer_conv2d())
            self.b1 = tf.get_variable('b1', [classnum],initializer=tf.constant_initializer(0.0))

    def inference(self,images):
        images=tf.cast(images,tf.float32)/255.0
        out = tf.matmul(images, self.w1)+self.b1
        return out
